...
$sql2="SELECT * FROM reservation WHERE id='$id' and dot>'".date("Y-m-d")."';";
    	...
    	$result2=mysqli_query($conn,$sql2);
				...
				while ($row = mysqli_fetch_assoc($result2)) { // Important line !!! Check summary get row on array ..
					...
					$sql3 = "SELECT * FROM traindisplay WHERE trainid='".$row['trainid']."';";
				$result3 = mysqli_query($conn, $sql3);
				$row1=mysqli_fetch_assoc($result3);
				$trainname=$row1['trainname'];
				$fromstn=$row1['fromstn'];
				$tostn=$row1['tostn'];
				$amount=$row1['amount'];
					echo "<tr>";
						echo "<td>".$row['name']."</td>";
						...
						echo "<td>".$row['dot']."</td>";
						echo "<td>".$row['dob']."</td>";
						echo "<td>". $row1['trainname']."</td>";
						echo "<td>". $row1['fromstn']."</td>";
						echo "<td>".$row1['tostn']."</td>";
						echo "<td>".$row1['amount']."</td>";

						if (isset($_POST['upcmng'])){
							echo "<form action='./upcmng.php' method=post><input type='hidden' name='reserveid' value='".$row['reserveid']."'>";
