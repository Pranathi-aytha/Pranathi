...
//if (isset($_POST['train'])){
	//$result=mysqli_query("SELECT amount FROM traindisplay WHERE trainno=trainno");
...
if(isset($_POST['gnd'])){
	...
	$sql=mysqli_query("SELECT id FROM usertable WHERE email='$_COOKIE[username]'");
	$result=mysqli_query($conn,$sql);
                $row=mysqli_fetch_assoc($result);
$sql = "INSERT INTO `reservation` (`reserveid`,'name','age','gender' `remail`, `dot`, `dob`) VALUES (NULL,'$username','$age','$gender','$email','$dot','$dob');";
        if ($conn->query($sql)) {
